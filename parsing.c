...
/*   Updated: 2026/05/02 18:14:31 by rvaz-da-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
    ...
    while (i < ac - 1)
    {
        ...
        while(av[i][j])
        {
            if (!ft_isdigit(av[i][j]))
                return (fprintf(stderr, "Parsing Error: argument is not a valid positive integer\n"), false);
            j++;
        }
        i++;
    }
    if (strcmp(av[i], FIFO) && strcmp(av[i], EDF))
        return (fprintf(stderr, "Parsing Error: Last argument only accepts 'fifo' or 'edf'\n"), false);
    return (true);
}
...
t_args  init_data(char *av[])
{
    ...
    if (!args.scheduler)
        return (fprintf(stderr, "Allocation Error: scheduler type arg"), (t_args){0});
    return (args);
}